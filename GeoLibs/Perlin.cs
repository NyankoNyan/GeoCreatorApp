...
namespace GeoLibs
{
    internal class Perlin
    {
        ...
        static Perlin()
        {
            p = new int[hashSize * 2];
            for (int x = 0; x < hashSize * 2; x++) {
                p[x] = permutation[x % hashSize];
            }
        }
        ...
        public double GetNoise(double x, double y, double z)
        {
            ...
            aaa = CoordHash( xi, yi, zi );
            aba = CoordHash( xi, Inc( yi ), zi );
            aab = CoordHash( xi, yi, Inc( zi ) );
            abb = CoordHash( xi, Inc( yi ), Inc( zi ) );
            baa = CoordHash( Inc( xi ), yi, zi );
            bba = CoordHash( Inc( xi ), Inc( yi ), zi );
            bab = CoordHash( Inc( xi ), yi, Inc( zi ) );
            bbb = CoordHash( Inc( xi ), Inc( yi ), Inc( zi ) );
        ...
        private int CoordHash(int x, int y, int z)
        {
            return p[p[p[x] + y] + z];
        }
